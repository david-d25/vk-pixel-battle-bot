...
export default class AdminsCommand extends Command {
    ...
    private async handleList(message: VkMessage): Promise<void> {
        ...
        let response = `Над Pixel Battle имеют власть:\n`;
    ...
    private getUsage(): string {
        let result = '';
        result += 'Команды:\n';
        result += `/${this.triggerWord} admins\n`;
        result += `/${this.triggerWord} admins help\n`;
        result += `/${this.triggerWord} admins add (тег)\n`;
        result += `/${this.triggerWord} admins remove (тег)\n`;
        return result;
    }
}